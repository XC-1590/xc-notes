...
| 工具 | 用途 |
|------|------|
| read_screen(model, width) | 截屏 → 硅基流动 OCR → 屏幕文字（默认 GLM-4.5V，width 默认 960，可传别的模型） |
| click(x, y) | 点屏幕坐标 |
| press_key(key) | 按键：enter / space 等 |
| wait(seconds) | 等待 |
| screenshot() | 截图返回图片 |
| screen_size() | 屏幕分辨率 |

**文字游戏节奏：** click(1280,800) → wait 0.5 → read_screen(width=960)。
**选项出现就停手问她**（OCR 读不到坐标）。鼠标别放屏幕左上角（pyautogui fail-safe 会拒绝操作）。
